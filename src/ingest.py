...
async def process_document(file_path: Path) -> Tuple[int, int]:
    ...
    try:
        ...
        doc = Document(page_content=normalized_text, metadata={"source": file_path.name})
...
async def insert_chunks(docs: List[Document], vectors: List[List[float]]) -> None:
    ...
    if not docs or not vectors or len(docs) != len(vectors):
        raise ValueError("Invalid input: docs and vectors must be non-empty and same length")
    ...
    async with get_es_client() as client:
        ...
        try:
            ...
            logger.info(f"Bulk insert completed: {success} succeeded, {n_failed} failed")
            
            if n_failed > 0:
                raise Exception(f"Bulk insert failed: {n_failed} documents failed to index")
