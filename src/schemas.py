...
class QuestionResponse(BaseModel):
    answer: str = Field(
        ..., description="The answer to the question based on the documents"
    )
...
class DocumentResponse(BaseModel):
    ...
    total_chunks: int = Field(
        ..., description="Total number of chunks created from the documents"
    )
    failed_files: List[str] = Field(
        default=[], description="List of files that failed to process"
    )
